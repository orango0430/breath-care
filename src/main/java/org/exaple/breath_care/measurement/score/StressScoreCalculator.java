 ...
 *
 * <p><b>지금은 아무 데서도 부르지 않는다.</b> 화면에 내보내는 지표를 컨디션 지수로 바꾸면서
 * ({@link ConditionScoreCalculator}, V14) 이 계산은 연결을 끊었다. 개인 기준선 대비 z-score라
 * 측정 5회를 채우기 전에는 계속 null이었고, 행사에서 한 번씩 측정하는 사람에게는
 * 영영 숫자가 안 나오기 때문이다.
 *
 * <p>지우지 않고 남겨 둔 이유는 "내 평소보다 높은가"가 절대 공식보다 나은 접근이라서다.
 * 사용자가 꾸준히 측정하는 단계가 오면 컨디션 지수와 나란히 되살린다.
 *
