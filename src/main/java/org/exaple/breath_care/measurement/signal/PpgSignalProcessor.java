...
public class PpgSignalProcessor implements SignalProcessor {
     ...
     */
    /// 1.4에서 1.25로 내렸다. 실기기 파형은 봉우리 높이가 박동마다 고르지 않아서,
    /// 문턱이 조금만 높아도 작은 박동이 안 잡히고 "피크부족"으로 떨어진다.
    ///
    /// <p>1.1까지 더 내려 봤다가 되돌렸다. 실기기에서 1.25로는 되던 측정이 1.1에서
    /// 안 됐다. 내릴수록 잡음을 봉우리로 줍고, 가짜 봉우리는 간격을 흩뜨려서 오히려
    /// 더 많이 버려지게 만든다. 못 잡은 박동만 문제가 아니라 없는 박동을 잡는 것도
    /// 같은 크기의 문제다. [튜닝 대상]
